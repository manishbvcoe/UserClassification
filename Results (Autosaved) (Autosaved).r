...
Temp <- TargetUser[TargetUser$CUGMO <=2 , ]
dim(Temp)
Business <- union(Business$IMSI,Temp$IMSI)
length(Business)
BCUGMO <-Temp$IMSI
...
Familylst <- list(FStayDuration, FFbyAllNMO_TAPIN, FMOperDay_TAPIN, FUniqFrMo)
...
FamilylstName <- list("FStayDuration", "FFbyAllNMO_TAPIN", "FMOperDay_TAPIN", "FUniqFrMo")
...
write.csv(Chart,file="chart_modified_addndays_21.csv")
print(sum)
...
i1 <- intersect(BStayDuration,FMOperDay_TAPIN )
i2 <- intersect(BStayDuration,FFbyAllNMO_TAPIN)
i3 <- intersect(BNCountry_Called ,FStayDuration)
i4 <- intersect(BUniqFrMo ,FStayDuration)
...
i1User <- TargetUser[TargetUser$IMSI %in% i1,] 
i2User <- TargetUser[TargetUser$IMSI %in% i2,] 
i3User <- TargetUser[TargetUser$IMSI %in% i3,] 
i4User <- TargetUser[TargetUser$IMSI %in% i4,] 

#IMSI NCountry_Called CUGMO UniqFrMo        UNR  BbyAllMt    MobyMt PerCallGrtEq120sec NMO_FrPerDay_TAPIN BbyAllMo_Fr_TAPIN StayDuration FbyAllNMO_TAPIN NumTrips NMOByNMT_TAPIN
#Temp <- i1User[,c(1,56,10,3,26,27,25,30,50,38,54,44,53,41)]

i1User <- i1User[,c(1,56,10,3,26,27,25,30,50,38,54,44,53,41,42)]
i2User <- i2User[,c(1,56,10,3,26,27,25,30,50,38,54,44,53,41,42)]
i3User <- i3User[,c(1,56,10,3,26,27,25,30,50,38,54,44,53,41,42)]
i4User <- i4User[,c(1,56,10,3,26,27,25,30,50,38,54,44,53,41,42)]
...
#for first set
#Business
Temp <- union(BNCountry_Called, BCUGMO)
Temp  <- union(Temp, BCUGMO)
Temp  <- union(Temp, BUniqFrMo)
Temp  <- union(Temp, BUNR)
Temp  <- union(Temp, BBbyAllMt)
Temp  <- union(Temp, BPerCallGrtEq120sec)
Temp  <- union(Temp, BNumTrips)
Temp  <- union(Temp, BNMO_FrPerDay_TAPIN)
Temp  <- union(Temp, BBbyAllMo_Fr_TAPIN)
Business_1 <- Temp[Temp %in% i1User$IMSI] 

#Family
Temp <- union(FStayDuration, FFbyAllNMO_TAPIN)
Temp  <- union(Temp, FUniqFrMo)
Family_1 <- Temp[Temp %in% i1User$IMSI]
